...
async def update(time_end_raw, userid):
    query = """UPDATE default_table set time_of_end = %s WHERE user_id LIKE %s AND time_of_end IS NULL"""

    cur.execute(query, (time_end_raw, userid))
    conn.commit()
...
async def output(userid, time_end_raw):
    query = """SELECT * FROM default_table WHERE user_id LIKE %s AND time_of_end LIKE %s"""

    cur.execute(query, (userid, time_end_raw))
    for row in cur:
        ...
        out_time_end = row['time_of_end']
        out_date = row['date_job']
        return out_name, out_id, out_destination, out_time_begin, out_time_end, out_date
    conn.commit()
